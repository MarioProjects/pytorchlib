...
def select_model(model_name, dropout=0, ruido=0, gray=0, growth_rate=0, in_features=0, out_features=0, out_type='relu', data_parallel=False):
    ...
    if 'VGG' in model_name:
        ...
        my_model = MobileNetV2(model_name, dropout, ruido, gray).cuda()
    elif 'MiMic' in model_name:
        my_model = BasicModel(model_name, in_features, out_features, out_type).cuda()
...
def load_model(model_name, states_path, gray, dropout=0.0, ruido=0.0, growth_rate=0, in_features=0, out_features=0, out_type='relu'):

    if not os.path.exists(states_path): assert False, "Wrong Models States Path!"
    my_model = select_model(model_name, dropout, ruido, gray, growth_rate=growth_rate, in_features=in_features, out_features=out_features, out_type=out_type)
