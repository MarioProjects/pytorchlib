...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, num_classes=1000, input_channels=3):
        """
        Parameters
        ----------
...
def SeResNeXtModel(configuration_blocks, configuration_maps="", input_channels=0, block_type="Bottleneck", cardinality=32, num_classes=2, flat_size=0, pretrained=False):
    my_model = False
    ...
    """
    if "Bottleneck" in block_type: block_type = SEResNeXtBottleneck
    else: assert False, "No block type '"+str(block_type)+"' allowed!"
    if pretrained == True: aux_num_classes = 1000
    else: aux_num_classes = num_classes
    model = SENet(block_type, configuration_blocks, groups=32, reduction=16,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=aux_num_classes, input_channels=input_channels)
    if pretrained is not False:
        settings = pretrained_settings['se_resnext101_32x4d']['imagenet']
        initialize_pretrained_model(model, aux_num_classes, settings)
