...
def apply_DeConv(inp,out,kernel,act,shape=None,std=0.0,drop=0.0,bn=True,stride=1,padding=1,output_padding=0):
    ...
    forward_list=[conv]
    if bn:
        forward_list.extend(convBN)
    if std!=0.0:
        forward_list.extend(nlayer)

    forward_list.extend(activation)
    if drop!=0:
        forward_list.extend(dropl)
...
def apply_conv(inp,out,kernel,act,shape=None,std=0.0,drop=0.0,bn=True,stride=1,padding=1):
    ...
    forward_list=forward_list.extend(activation)
...
def one_hot_target(t,max_val):
    #this functions takes as input a vector with a number indicating the label and return a one-hot matrix with a one in the position of the label
    a,b=(t.size(0),max_val)

    if t.is_cuda:
        y = torch.zeros((a,b)).cuda()
    else:
        y = torch.zeros((a,b))
